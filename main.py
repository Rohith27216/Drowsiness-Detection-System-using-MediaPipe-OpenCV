...
try:
    ...
    print("FaceLandmarker init failed, falling back to Haar cascades:", e)
...
while True:
    ...
    if use_task_model and results.face_landmarks:
    ...
    elif not use_task_model:
        # Fallback using Haar cascades (approximate EAR from eye bounding boxes)
        ...
        if len(faces) > 0:
            ...
            eyes_sorted = sorted(eyes, key=lambda e: e[0]) if len(eyes) > 0 else []
            ...
            if len(ratios) == 1:
                ...
                avg_ear = (ratios[0] + ratios[1]) / 2.0

            eye_closed = False

            # threshold tuned for bounding-box-based ratio
            if avg_ear > 0 and avg_ear < 0.25:
                eye_closed = True
                closed_frames += 1
        ...
        perclos = (closed_frames / max(total_frames, 1)) * 100
